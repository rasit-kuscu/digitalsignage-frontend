    ...
    return {
        ...
        execute: function() {
            RoleEditComponent = (function () {
                function RoleEditComponent(_roleService, _privilegeService, formBuilder, _notificationService, router, activatedRoute) {
                ...
                RoleEditComponent.prototype.ngOnInit = function () {
                    ...
                    Observable_1.Observable.forkJoin(this._roleService.detail(this.roleId), this._privilegeService.load()).subscribe(function (response) {
                        if (response[0].status === 'success' && response[1].status === 'success') {
                            _this.role = response[0].data;
                            _this.privilegeList = response[1].data;
                ...
                RoleEditComponent.prototype.prepareForm = function () {
                    this.editRoleForm = this.formBuilder.group({
                        'name': [this.role['name'], forms_1.Validators.required],
                ...
                RoleEditComponent.prototype.populateCheckBoxes = function () {
                    for (var _i = 0, _a = this.privilegeList; _i < _a.length; _i++) {
                        var privilege = _a[_i];
                        var index = this.role['privileges'].map(function (x) { return x.id; }).indexOf(privilege.id);
                        if (index >= 0) {
                        ...
                        this.formPrivilegeArr[privilege.id] = new forms_1.FormControl(true);
                    }
                };
                RoleEditComponent.prototype.updateCheckedOptions = function (option, event) {
                    if (option.isChecked) {
                ...
                RoleEditComponent.prototype.save = function () {
                    ...
                    this.isSaving = true;
                    this._roleService.update(new role_1.Role(this.roleId, this.editRoleForm.value.name, privileges))
                        .subscribe(function (response) {
                        _this.isSaving = false;
                        if (response.status === 'success') {
                            _this._notificationService.success('İşlem Başarılı', 'Yetki bilgileri güncellendi.', {});
                        }
